...
describe("Todo List Application", () => {
  beforeEach(() => {
    ...
    // Inject the HTML into the global document body
    ...
    // Clear the require cache so script.js runs fresh with the new DOM
    jest.resetModules();

    // Require the script after the DOM is ready
    const { initializeTodoApp } = require("../script.js");
    initializeTodoApp();
  });
  ...
  test("should load todos from localStorage on page load", () => {
    localStorage.getItem.mockReturnValue(
      JSON.stringify([{ text: "Loaded Task", completed: false }])
    );
    jest.resetModules();
